...
esp_err_t digipot_init(digipot_handle_t *digipot_handle, digipot_type_t digipot_type, gpio_num_t sda_gpio, gpio_num_t scl_gpio, size_t clock_speed) {
    ...
    esp_err_t err = i2c_master_bus_add_device(bus_handle, &digipot_handle.dev_cfg, dev_handle);
    if (err != ESP_OK) {
...
esp_err_t digipot_register_write(uint8_t *tx_buf, uint8_t tx_buf_size, digipot_handle_t *digipot_handle) {
    ...
    return i2c_master_transmit(digipot_handle->i2c_port, digipot_handle->i2c_addr, tx_buf, tx_buf_size, digipot_handle.transaction_timeout_ms / portTICK_PERIOD_MS);
}
